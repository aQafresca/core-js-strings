...
function removeFirstOccurrences(str, value) {
  const string = str.indexOf(value);
  if (string === -1) {
    return str;
  }
  return str.slice(0, string) + str.slice(string + value.length);
}
...
function removeLastOccurrences(str, value) {
  const string = str.lastIndexOf(value);
  if (string === -1) {
    return str;
  }
  return str.slice(0, string) + str.slice(string + value.length);
...
function startsWith(str, substr) {
  const string = str.startsWith(substr);
  return string;
}
...
function endsWith(str, substr) {
  const string = str.endsWith(substr);
  return string;
}
...
function countVowels(str) {
  ...
  return newStr.filter((item) => vowels.includes(item)).length;
}
...
function isPalindrome(str) {
  const string = str.toLowerCase().replace(/[^а-яa-z0-9]/g, '');
...
function findLongestWord(sentence) {
  const string = sentence.match(/\w+/g) || [];
  return string.reduce((longestWord, currentWord) => {
...
function reverseWords(str) {
  ...
  return splitWord.map((word) => word.split('').reverse().join('')).join(' ');
}
